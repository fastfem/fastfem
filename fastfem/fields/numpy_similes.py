...
def moveaxis(
    ...
    coefs = jnp.moveaxis(
        field.coefficients,
        field._axis_field_to_numpy(source),
        field._axis_field_to_numpy(destination, out_of_bounds_check=False),
    )
    shapes = {
    ...
    shapes[destination[0]] = (
        destshape[: destination[1]] + (rem_axis,) + destshape[destination[1] :]
    )
