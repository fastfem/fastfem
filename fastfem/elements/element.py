...
class StaticElement2D(Element2D):
    """
    Handles the management of element operations on an element with a preset
    position field. This is useful for mesh elements, where the position field is
    directly retrieved from the mesh.
    """
    ...
    def ref_to_real(
        ...
        """Evaluates the position field at (X,Y) in reference coordinates.
        The result is an array of values `pos(X,Y)`.
        ...
        Returns:
            NDArray: The interpolated values `pos(X,Y)`
        """
        return self.interpolate_field(self.position_field,X,Y)
    ...
    def compute_field_gradient(
        self,
        field: FieldType,
        pos_field: FieldType | None = None,) -> FieldType:
        """
        Calculates the gradient of a field f, returning a new field.
        The result is an array of shape `(*basis_shape,...,*fieldshape,2)`,
        where field is of shape `(*basis_shape,...,*fieldshape)`.
    ...
    def compute_field_gradient(
        ...
        Args:
            field (Field): an array of shape (*basis_shape,...,*fieldshape)
                representing the field to be interpolated.
            use_local_coords (bool): When true, the gradient is taken with respect
                to local coordinates. Otherwise, the gradient will be computed in
                Cartesian coordinates, as specified by the position field
    ...
    def compute_field_gradient(
        self,
        field: FieldType,
        coords_or_flag = False,
    ) -> FieldType:
        if isinstance(coords_or_flag,bool):
            return super().compute_field_gradient(field,None if coords_or_flag else self.position_field)
        else:
            return super().compute_field_gradient(field,coords_or_flag)
